...
def _resolve_plot_path_nodes(gg):
    path_nodes = getattr(gg, "shortest_path_nodes", None)
    if path_nodes is not None:
        return list(path_nodes)

    shortest_path_attr = getattr(gg, "shortest_path", None)
    if shortest_path_attr is not None and not callable(shortest_path_attr):
        shortest_path = list(shortest_path_attr)
        if len(shortest_path) > 0 and shortest_path[0] == "initial":
            return shortest_path[1:-1]
        return shortest_path

    node_wise = getattr(gg, "node_wise_shortest_path", None)
    if node_wise is not None and not callable(node_wise):
        return list(node_wise)
...
def plot_ds_set_gaussians(
    ds_set,
    config,
    include_points=False,
    ax=None,
    file_name=None,
    include_trajectory=None,
    save_as=None,
):
    ...
    if include_trajectory is not None:
        include_points = bool(include_points) or bool(include_trajectory)
...
def plot_demonstration_set(demo_set, config, ax=None, file_name=None, save_as=None):
