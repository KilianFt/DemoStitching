...
class DAMM:
    def __init__(self, x: np.ndarray, x_dir: np.ndarray, nu_0: float = 5, kappa_0: float = 1, psi_dir_0: float = 1, rel_scale: float = 0.7, total_scale: float = 1.5):
        self.x = x
        self.x_dir = x_dir
    ...
    @staticmethod
    def pre_process(x: np.ndarray, x_dot: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        x_dot_norm = np.linalg.norm(x_dot, axis=1)
        mask = x_dot_norm != 0
        x = x[mask]
        x_dot = x_dot[mask]
        x_dir = x_dot / x_dot_norm[mask][:, None]
        return x, x_dot, x_dir
    ...
    def extract_gaussian(z: np.ndarray, x: np.ndarray, rel_scale=0.7, total_scale=1.5) -> list:
        ...
        Sigma = np.array([adjust_cov(np.cov(x[z == k].T), rel_scale=rel_scale, total_scale=total_scale) for k in range(K)])
